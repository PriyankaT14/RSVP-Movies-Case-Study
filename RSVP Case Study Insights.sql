USE imdb;
--  selecting the database which has the tables.
...
WITH null_values_info AS
(
   SELECT
      'id' AS 'Null_Column_Name',
   ...
   UNION ALL
   SELECT
      'date_published' AS 'Null_Column_Name',
      COUNT(*) AS null_values 
   FROM
      movie 
   WHERE
   ...
   UNION ALL
   SELECT
      'worlwide_gross_income' AS 'Null_Column_Name',
      COUNT(*) AS null_values 
   FROM
      movie 
   WHERE
   ...
   UNION ALL
   SELECT
      'production_company' AS 'Null_Column_Name',
      COUNT(*) AS null_values 
   FROM
      movie 
   WHERE
...
/** OBSERVATIONS **/
-- Drama(1078) genre has highest number of movies produced last year.
...
WITH single_genre_movie_table AS 
(
   SELECT
      movie_id,
...
SELECT
   COUNT(*) AS number_of_movies_in_one_genre 
FROM
   single_genre_movie_table;
